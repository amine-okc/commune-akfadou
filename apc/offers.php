...
<head>
    ...
    <title>Appels d'offre - Commune d'Akfadou</title>
    ...
    <div class="container">
        ...
        <div class="row offers" style="margin-top: 4%">
            ...
            <div class="col-md-8">
                ...
                $total_pages_sql = "SELECT COUNT(*) FROM appels";
                $result = mysqli_query($connection, $total_pages_sql);
                $total_rows = mysqli_fetch_array($result)[0];
                $total_pages = ceil($total_rows / $no_of_records_per_page);
                ?>
                    <div class="card mb-4 pagetitle p-3 rounded">
                        <h1 class="page-title">Appels d'offre</h1>
                    </div>
                <?php if ($pageno <= $total_pages) { ?><label class="float-right" style="color : grey">Page <?php echo $pageno ?></label><br><br>
                <?php }


                $sql = "SELECT * , DATE_FORMAT(date, \"%d-%m-%Y\") AS date, DATE_FORMAT(date_exp, \"%d-%m-%Y\") AS date_exp  FROM `appels` ORDER BY id DESC LIMIT $offset, $no_of_records_per_page";
                ...
                ?>
                    ...
                    <div class="card mb-4 shadow p-3 bg-white rounded " data-aos="fade-up">
                        <div class="card-body">
                            ...
                            <?php if ($row['file'] != '') { ?><a style="text-decoration : none;color : rgb(13,110,253)!important;" href="../assets/appels/<?php echo $row['file'] ?>" target="_blank">Détails &nbsp; <i class="fas fa-file-alt"></i></a><?php } ?>
				<?php if ($row['date_exp'] != NULL) { ?><div class="small text-muted float-right">Date d'expiration : <?php echo $row['date_exp'] ?></div> <?php } ?>
                        </div>
                    </div><br>
